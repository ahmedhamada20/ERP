...
@section('title', 'سند صرف ' . $voucher->number)
@section('page_title', 'سند صرف ' . $voucher->number)
...
<style>
    ...
    .v-amount { background:linear-gradient(135deg, #fee2e2, #fecaca); border:1px solid #fca5a5; }
    .v-amount .v { color:#b91c1c; font-size:1.7rem; }
...
<div class="row g-3 mb-3">
    ...
    <div class="col-md-3">
        <div class="v-info">
            ...
            <div class="v">
                @if($voucher->isPosted())
                    <span class="badge bg-danger px-3 py-2"><i class="bi bi-check-circle"></i> مرحّل (صرف)</span>
                @elseif($voucher->isCancelled())
                    <span class="badge bg-secondary px-3 py-2"><i class="bi bi-x-circle"></i> ملغي</span>
                @else
    ...
    <div class="col-md-6">
        <div class="v-info">
            <h6>الخزينة / البنك (المنصرف منها)</h6>
    ...
    @if($voucher->supplier)
    <div class="col-12">
        <div class="v-info" style="background:linear-gradient(135deg, #fef3c7, #fffbeb); border-color:#fde68a;">
            <h6><i class="bi bi-building"></i> سداد لمورد</h6>
            <div class="v">
                <a href="{{ route('admin.suppliers.show', $voucher->supplier) }}">
                    <code class="text-warning">{{ $voucher->supplier->code }}</code> — {{ $voucher->supplier->name }}
                </a>
                <span class="badge bg-secondary ms-2">{{ $voucher->supplier->type_label }}</span>
                @if($voucher->supplierInvoice)
                    <span class="ms-3">·
                        <i class="bi bi-link-45deg"></i> فاتورة:
                        <a href="{{ route('admin.supplier_invoices.show', $voucher->supplierInvoice) }}">
                            <code>{{ $voucher->supplierInvoice->number }}</code>
                        </a>
                        ({{ number_format($voucher->supplierInvoice->total, 2) }} {{ $voucher->supplierInvoice->currency }})
                    </span>
                @endif
            </div>
        </div>
...
<div class="card">
    <div class="card-body d-flex gap-2 justify-content-between flex-wrap">
        ...
        <div class="d-flex gap-2">
            <a href="{{ route('admin.accounting.vouchers.payments.print', $voucher) }}" target="_blank" class="btn btn-outline-primary">
...
<div class="modal fade" id="cancelModal" tabindex="-1">
    <div class="modal-dialog">
        <form action="{{ route('admin.accounting.vouchers.payments.cancel', $voucher) }}" method="POST" class="modal-content">
            @csrf
            <div class="modal-header bg-danger text-white">
                <h5 class="modal-title"><i class="bi bi-x-circle"></i> إلغاء سند الصرف</h5>
